    ...
    */
	...
	for(int i=num_forks; i>0; i--){
		...
		if(pid<0){
			...
			if(execl("/home/cs392/shared/cs392/lab7/randomsleep", "/home/cs392/shared/cs392/lab7/randomsleep", (char*) NULL)==-1){
	...
	for(int j=num_forks; j>0; j--){
		if(pid>0){
			...
			if(wait(&status)==-1){
				...
				printf("Child with pid <%ld> had terminated.\n", (long)pid);
			}
		}
	}
