...
void *sum_array(void *ptr) {
	...
	int retval;
	int partial_sum;
	for(int i = x.start_index; i < x.length; i+=2){
		partial_sum+=array[i];
		}
...
int main(int argc, char *argv[]) {
    ...
    targs[0].length = atoi(argv[1]);
    targs[1].start_index = 1;
    targs[1].length = atoi(argv[1]);
    
    if ((retval =
            pthread_create(&threads[0], NULL, sum_array, &targs[0])) != 0) {
        fprintf(stderr, "Error: Cannot create thread 1. %s.\n", 
                strerror(retval));
        return EXIT_FAILURE;
    }
    
    if ((retval =
            pthread_create(&threads[1], NULL, sum_array, &targs[1])) != 0) {
        fprintf(stderr, "Error: Cannot create thread 2. %s.\n", 
                strerror(retval));
        return EXIT_FAILURE;
    }
    */
    for (int i = 0; i < 2; i++){
    	...
    	targs[i].length = atoi(argv[1]);
    	
    
    if ((retval =
            pthread_create(&threads[i], NULL, sum_array, &targs[i])) != 0) {
