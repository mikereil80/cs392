#include <pthread.h> // -lpthread instead of -l in makefile cause of pthreading
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
void *sleep_print(void *ptr){
	...
	srand((time(NULL)&OxFFFF)|(tid<<16));
	
	useconds_t sleep_time=(useconds_t)randdm_int_in_range(1+SCALE_FACTOR, 5*SCALE_FACTOR);
...
int main(int argc, char *argv[]){
	...
	for(int i=0; i<num_threads; i++){
		...
		if((retval=pthread_create(&threads[i],NULL,sleep_print,(void *)(&thread_nums[i])))!=0){
			fprintf(stderr, "Error: Cannot create thread %d. " "No more threads will be created. %s.\n", i+1, strerror(retval));
			break;
		}
		num_started++;
		if((retval=pthread_mutex_lock(&lock))!=0){
	...
	for(int i=0; i<num_started; i++){
		if(pthread_join(threads[i], NULL)!=0){
			fprintf(stderr, "Warning: Thread %d did not joinh properly.\n", thread_nums[i]);
		}
	}
	...
	if((retval=pthread_mutex_destory(&lock))!=0){
